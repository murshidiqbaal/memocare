...
//   ✅ Token fetch → correct Supabase table (profiles OR patients)
...
class FCMService {
  ...
  /// Saves token to correct Supabase table (profiles or patients).
  ...
  Future<void> _saveToken(String token) async {
    ...
    // 1. Try profiles with role 'caregiver'
    ...
    if (caregiverRow != null) {
      await _supabase
          ...
          .eq('user_id', userId)
          .eq('role', 'caregiver');
      debugPrint('[FCM] Token saved → profiles (caregiver)');
      return;
    }
