...
class _PatientSOSScreenState extends ConsumerState<PatientSOSScreen>
  ...
  Future<void> _fireSOS() async {
    ...
    // Delegate to the SosController — it handles fetching location + DB insert
  ...
  void _onStateChange(AsyncValue<void>? prev, AsyncValue<void> next) {
    ...
    if (next is AsyncError) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('SOS failed: ${next.error}'),
      ...
      _fired = false;
      _showSuccessDialog(false); // wasQueued logic simplified for now
    }
  }
  ...
  Widget build(BuildContext context) {
    // Listen for controller state transitions
    ref.listen<AsyncValue<void>>(sosControllerProvider, _onStateChange);
